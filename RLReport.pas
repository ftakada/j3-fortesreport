...
uses
  DB, Classes, SysUtils, Math, Contnrs, TypInfo, Variants, Windows, Types,
  ExtCtrls, DBCtrls, Controls, Forms, Dialogs, StdCtrls, Messages, Buttons,
  Graphics, Mask, MaskUtils, RLMetaVCL, RLMetaFile, RLFeedBack, RLParser,
  RLFilters, RLConsts, RLUtils, RLPrintDialog, RLSaveDialog,
  RLPreviewForm, RLPreview, RLTypes, RLPrinters;
...
type
  ...
  private
    function  GetAsCurrency: Extended;
    procedure SetAsCurrency(Value: Extended);
  public
    property AsCurrency: Extended read GetAsCurrency write SetAsCurrency;
  published
...
begin
  ...
  try
    dialog.DefaultExt :=String(FormatFileExt(RLFILEEXT));//bds2010
    dialog.Filter     :=String(AddFileFilter('',CS_ProductTitleStr,RLFILEEXT));//bds2010
...
begin
  r := FindParentReport;
  if Assigned(r) and (r.ReportState in [rsWriting,rsClosing]) then
  else
  begin
    InternalPaint;
    InternalPaintFinish;
...
procedure TRLCustomControl.DoBeforePrint(var aPrintIt: Boolean);
  procedure SetParentDBTextDS(CompParent: TWinControl);
  begin
    if CompParent <> nil then
    begin
      if (CompParent is TRLCustomSkipper) then
        (Self as TRLDBText).DataSource := (CompParent as TRLCustomSkipper).DataSource
      else
        SetParentDBTextDS(CompParent.Parent);
    end;
  end;
  procedure SetParentDBMemoDS(CompParent: TWinControl);
  begin
    if CompParent <> nil then
    begin
      if (CompParent is TRLCustomSkipper) then
        (Self as TRLDBMemo).DataSource := (CompParent as TRLCustomSkipper).DataSource
      else
        SetParentDBMemoDS(CompParent.Parent);
    end;
  end;
begin
  if (Self is TRLDBText) and ((Self as TRLDBText).DataSource = nil) then
    SetParentDBTextDS(Self.Parent)
  else if (Self is TRLDBMemo) and ((Self as TRLDBMemo).DataSource = nil ) then
    SetParentDBMemoDS(Self.Parent);
...
begin
  ...
  begin
    ...
    TextRectEx(r,l,t,AnsiString(s),ToMetaTextAlignment(TAlignment(Alignment)),ToMetaTextLayout(TTextLayout(Layout)),f);//bds2010
  end;
end;
...
begin
  ...
  aSize.X:=0;
  aSize.Y:=0;
  with TextBounds(AnsiString(c)+' ',Self.Font,0) do //bds2010
  begin
    Inc(aSize.X,X);
    Inc(aSize.Y,Y);
  end;
...
begin
  ...
  aSize.X:=0;
  aSize.Y:=0;
  with TextBounds(AnsiString(c)+' ', Self.Font,fAngle) do //bds2010
  begin
    Inc(aSize.X,X);
    Inc(aSize.Y,Y);
  end;
    ...
    if Assigned(d) and d.Active and not d.Eof then
      ...
      begin
        ...
        begin
          ...
          begin
            if TryStrToFloat(f.DisplayText,N) then
              Result := FormatFloat(fDisplayMask,N)
            else if TryStrToDateTime(f.DisplayText,dDate) then
              Result := FormatDateTime(fDisplayMask,dDate)
            else
              Result := FormatMaskText(fDisplayMask,f.DisplayText);
          end;
        end;
      end
      else if fDataFormula<>'' then
...
{$else}    
  if VarIsNumeric(fieldvalue) then
    fSum:=fSum+fieldvalue
  //Sysrs tratamento para ftFMTBcd
  else if Assigned(fieldref) and (fieldref is TNumericField) then
    fSum:=fSum + fieldref.AsCurrency;
{$endif}
...
begin
  ...
  begin
    ...
    Y:=ARect.Top;
    (Canvas as TRLGraphicSurface).TextRectEx(ARect,X,Y,AnsiString(Text),ToMetaTextAlignment(TAlignment(Alignment)),ToMetaTextLayout(TTextLayout(tlTop)),MetaTextFlagIntegralHeight or MetaTextFlagAutoSize);//bds2010
  end
  else if Canvas is TCanvas then
...
begin
  ...
  Width      :=185;
  Height     :=89;
  Behavior   :=[beSiteExpander]; //Sysrs estava [], mas alterei para ficar igual a versão do BDS2006
...
begin
  ...
  if not IsPreparing then
    ...
    case fInfoType of
      ...
      itFullDate      : s:=FormatDateTime(FormatSettings.LongDateFormat,MasterReport.ReportDateTime);
...
begin
  ...
  begin
    ...
    BandType:=aType;
    Height  :=20;
    Name    :=String(NewComponentName(Result));//bds2010
  end;
end;
...
begin
  ...
  PrintParams.ToPage := Pages.PageCount;
...
function TRLLabel.GetAsCurrency: Extended;
var
  S: String;
  N: Extended;
begin
...
procedure TRLLabel.SetAsCurrency(Value: Extended);
begin
