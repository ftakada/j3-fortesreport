...
//function IntToStr(const val: Int64): AnsiString; //celso //bds2010
...
//Criei assim para não precisar fazer AnsiTring(IntToStr()) em todos
function IntToStr(const val: Int64): AnsiString;
begin
   Result := AnsiString(SysUtils.IntToStr(val));
end;
...
function RTF_PenStyle(aPenStyle:TRLMetaPenStyle):AnsiString;
begin
  case aPenStyle of
...
function RTF_BrushStyle(aBrushStyle:TRLMetaBrushStyle):AnsiString;
begin    
  case aBrushStyle of
...
function RTF_BoundsRect(aLeft,aTop,aWidth,aHeight:integer):AnsiString;
begin
...
function RTF_Size(aWidth,aHeight:integer):AnsiString;
begin
  result:='\dpxsize'+IntToStr(PixelsToTwips(aWidth))+
          '\dpysize'+IntToStr(PixelsToTwips(aHeight));
end;

function RTF_PenColor(aRed,aGreen,aBlue:integer):AnsiString;
const
  prefix='dplineco';
begin
...
function RTF_BrushColor(aRed,aGreen,aBlue:integer):AnsiString;
const
  prefix='dpfillbgc';
begin
  result:='\'+prefix+'r'+IntToStr(aRed)+'\'+prefix+'g'+IntToStr(aGreen)+'\'+prefix+'b'+IntToStr(aBlue);
...
function RTF_PenWidth(aPenWidth:integer):AnsiString;
begin
  result:='\dplinew'+IntToStr(PixelsToTwips(aPenWidth));
end;

function RTF_FormatText(const aText:AnsiString):AnsiString;
var
  i:integer;
begin
  result:=aText;
...
var
  ...
  s:AnsiString;
begin
  s:=AnsiString(IntToHex(aInt,aWid));//bds2010
...
begin
  ...
  begin
    fTempStream.free;
    SysUtils.DeleteFile(String(fTempFileName));//bds2010
  end;
  //
  inherited;
end;
...
begin
  ...
  fTempStream:=TFileStream.Create(String(fTempFileName),fmCreate);
end;
  ...
  try
    ...
    try
      ...
      for i:=0 to fFontNames.Count-1 do
        StreamWriteLn(endstream,'\f'+IntToStr(i)+'\fnil\fcharset0 '+AnsiString(fFontNames[i])+';');//bds2010
...
var
    ...
    try
      ...
      try
        ...
        while str.Read(ch,1)>0 do
          WrapWrite(AnsiString(IntToHex(ch,2)),wrp);//bds2010
  ...
  begin
    ...
    Write('{');
    // seleciona fonte
    Write('\f'+IntToStr(fFontNames.IndexOf(String(aObj.Font.Name)))+'\fs'+IntToStr(aObj.Font.Size*2));//bds2010
    // efeitos
    ...
    if (aObj.Font.Style and MetaFontStyleStrikeOut)=MetaFontStyleStrikeOut then
      Write('\strike');
    // o texto
    Write(' '+RTF_FormatText(AnsiString(aObj.DisplayText)));//bds2010
    // retorna fonte
    if (aObj.Font.Style and MetaFontStyleStrikeOut)=MetaFontStyleStrikeOut then
      Write('\strike0');
    if (aObj.Font.Style and MetaFontStyleUnderline)=MetaFontStyleUnderline then
