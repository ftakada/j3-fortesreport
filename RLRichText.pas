...
uses
  Classes, SysUtils, Contnrs, Math, Windows, Types, Graphics, RLMetaVCL,
...
begin
  if Canvas is TCanvas then
    ...
    TRLGraphicSurface(Canvas).TextOut(X,Y,AnsiString(Text));//bds2010
end;
...
begin
  if Canvas is TCanvas then
    ...
    TRLGraphicSurface(Canvas).TextRect(Rect,X,Y,AnsiString(Text));//bds2010
end;

{ TRLRichFontList }

destructor TRLRichFontList.Destroy;
begin
  inherited Destroy;
end;
...
begin
  ...
  //
  FontList  := TRLRichFontList.Create(False);
...
begin
  repeat
    ...
    if not ReadCh(ch) then
      Break;
    if not CharInSet(ch, NullSet) then
    begin
      GotoBookmark(m);
      Break;
    end;
...
begin
  repeat
    ...
    if not ReadCh(ch) then
      Break;
    if not CharInSet(ch, SpaceSet) then
    begin
      GotoBookmark(m);
      Break;
    end;
...
begin
  ...
  try
    Word:='';
    while ReadCh(ch) and CharInSet(ch, AlphaSet) do
    begin
      Word    :=Word+ch;
      Bookmark:=GetBookmark;
    end;
...
begin
  ...
  try
    s:='';
    i:=0;
    while ReadCh(ch) and (CharInSet(ch, DigitSet) or (CharInSet(ch, ['+','-']) and (i=0))) do
    begin
      s:=s+ch;
      Inc(i);
      Bookmark:=GetBookmark;
...
begin
  ...
  Bookmark:=GetBookmark;
  try
    if ReadCh(Ch) and CharInSet(Ch, LiteralSet) then
      Result:=True;
  finally
    if not Result then
      GotoBookmark(Bookmark);
...
begin
  ...
  Bookmark:=GetBookmark;
  try
    if ReadCh(Ch) and CharInSet(Ch, TextualSet) then
      Result:=True;
  finally
    if not Result then
      GotoBookmark(Bookmark);
  ...
  try
    if ReadCh(c) and (c='\') then
      ...
      begin
        ...
        if ReadCh(h1) and CharInSet(h1, HexSet) then
          if ReadCh(h2) and CharInSet(h2, HexSet) then
          begin
  ...
  begin
    ...
    while i<=Length(s) do
      case s[i] of
        ...
        m:=i;
        while (i<=Length(s)) and not CharInSet(s[i], [#32,#9]) do
          Inc(i);
...
//  Result:=FontList[Index];
  while FontList.Count<Index+1 do
  begin
    i := FontList.Add(TRLRichFont.Create);
    FontList[i].Name :=DefaultFontName;
    FontList[i].Family :='nil';
    FontList[i].Charset:=0;
    FontList[i].Num  := FontList.Count;
  end;
  Result := FontList[Index];
end;
...
begin
  ...
  NewCRC:=CRC16(AnsiString(NewText));
  OldLen:=TextLength;
  OldCRC:=TextCRC;
...
constructor TRLRichStyle.Create;
begin

end;

destructor TRLRichStyle.Destroy;
begin
...
constructor TRLRichFont.Create;
begin

end;

destructor TRLRichFont.Destroy;
begin
