...
begin
  ...
  for i:=1 to Length(aText) do
    if Ord(aText[i])<=127 then
      s:=s+String(code39x[Ord(aText[i])])//bds2010
    else
      fInvalid:=True;
  Result:=GetAs39(s);
end;
...
begin
  Result:=-1;
  for i:=0 to High(Table_128) do
    if String(Table_128[i].c)=c then //bds2010
    begin
      Result:=i;
      Break;
    end;
...
begin
  ...
  case fBarcodeType of
    ...
    bcEAN128C: begin // special identifier FNC1 = function code 1 for EAN 128 barcodes
                 Result:=Result+Convert(String(Table_128[102].data));//bds2010
  ...
  begin
    ...
    begin
      ...
      Result:=Result+Convert(String(Table_128[idx].data));
    ...
    begin
      ...
      if idx<0 then
        idx:=Find_Code128AB(' ');
      Result:=Result+Convert(String(Table_128[idx].data));//bds2010
      Inc(checksum,idx*codeword_pos);
      Inc(codeword_pos);
    end;
  checksum:=checksum mod 103;
  Result:=Result+Convert(String(Table_128[checksum].data));//bds2010
  // stopcode
...
begin
  ...
  for i:=0 to Length(aText)-1 do
    if Ord(aText[i])<=127 then
      s:=s+String(Code93x[Ord(aText[i])])//bds2010
    else
      fInvalid:=True;
  Result:=GetAs93(s);
end;
...
begin
  ...
  begin
    ...
    Result:=Result+String(Table_MSI[aText[i]]);//bds2010
  end;
  ...
  Result:=Result+String(Table_MSI[Char(Ord('0')+checksum)]);//bds2010
  // stopcode
  Result:=Result+'515';
end;
...
begin
  Result:='51';
  for i:=1 to Length(aText) do
    Result:=Result+String(Table_PostNet[aText[i]]);//bds2010
  Result:=Result+'5';
end;
...
var
  i,idx:integer;
begin
  Result := Table_cb[FindCodabar('A')].data;
  Result := Result + '0';
  for i:=1 to Length(aText) do
  begin
