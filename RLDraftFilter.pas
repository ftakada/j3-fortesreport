...
type
  ...
  private
    ...
    function    PrintCode(const aCommand:AnsiString; aParameter:integer=0):AnsiString;
...
const
  ...
  DefaultFontSize=6;
  //
  AccentLetters:AnsiString='áàãâäÁÀÃÂÄéèêëÉÈÊËíìîïÍÌÎÏóòõôöÓÒÕÔÖúùûüÚÙÛÜçÇºª';
  NormalLetters:AnsiString='aaaaaAAAAAeeeeEEEEiiiiIIIIoooooOOOOOuuuuUUUUcCoa';
  AccentChars:AnsiString  ='''`~^¨''`~^¨''`^¨''`^¨''`^¨''`^¨''`~^¨''`~^¨''`^¨''`^¨,,__';
...
begin
  ...
  begin
    p:=Pos(aString[i],AccentLetters);
    if p>0 then
      Result[i]:=AnsiChar(NormalLetters[p]);//bds2010
  end;
end;
...
function TRLDraftFilter.PrintCode(const aCommand:AnsiString; aParameter:integer=0):AnsiString;
var
  s,chr:AnsiString;
  i:integer;
begin
  s:=AnsiString(fCommands.Values[String(aCommand)]);
  Result:='';
  if s='' then
    Exit;
  i:=0;
  repeat
    ...
    if chr='' then
      ...
      Result:=Result+AnsiString(Char(aParameter))
    else if LowerCase(String(chr))= '#l' then
      Result:=Result+AnsiString(Char(aParameter mod 256))
    else if LowerCase(String(chr))='#h' then
      Result:=Result+AnsiString(Char(aParameter div 256))
    else if chr='$' then
      Result:=Result+AnsiString(IntToStr(aParameter))
    else
      Result:=Result+AnsiString(Char(strtoint(String(chr))));
  until False;
end;
...
begin
  ...
  begin
    ob.Text:=AnsiString(TrimRight(String(ob.Text)));
...
const
  HorzTraceChars:array[TRLLineDrawMethod,Boolean] of Ansichar=((#32,#32),('-','='),(#196,#205));
  VertTraceChars:array[TRLLineDrawMethod,Boolean] of Ansichar=((#32,#32),('|','|'),(#179,#186));
var
...
begin
  ...
  begin
    ...
    for i:=0 to h-1 do
      AddSimpleText(AnsiString(gch),cpp,0,PixelToPinX(aObj.BoundsRect.Left-chw div 2),PixelToPinY(aObj.BoundsRect.Top+i*gh),aList);
  end;
end;
...
begin
  ...
  begin
    ReplCh:=HorzTraceChars[fLineDrawMethod,aObj.Pen.Width>1];
    aux   :=StringOfChar(AnsiChar(ReplCh),RectHorzLength);
    AddSimpleText(aux,TraceCPP,0,PixelToPinX(aObj.BoundsRect.Left),PixelToPinY(aObj.BoundsRect.Top)-OneLetterHeightInPins div 2,aList);
    if RectVertLength>1 then
      AddSimpleText(aux,TraceCPP,0,PixelToPinX(aObj.BoundsRect.Left),PixelToPinY(aObj.BoundsRect.Bottom)-OneLetterHeightInPins div 2,aList);
  end;
...
procedure StuffText(const aText:AnsiString; var aInto:AnsiString; aPos:integer);
var
...
begin
  ...
  aux    := String(aInto);
  if fill>0 then
    Insert(StringOfChar(#32,fill),aux,intolen+1);
  aInto := AnsiString(aux);
      ...
      begin
        ...
        begin
          ...
          for j:=0 to b.Width-1 do
            DeviceWrite(AnsiString(Char(Byte(GetBitmapPixel(b,j,0,clWhite)=clBlack)*128+
