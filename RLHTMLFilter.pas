...
uses
  SysUtils, Classes, Contnrs, Windows, Types, Graphics, RLMetaVCL,
...
begin
  ...
  Result:=String(ByteToHex(rgb.AsSplit.Red)+ByteToHex(rgb.AsSplit.Green)+ByteToHex(rgb.AsSplit.Blue));//bds2010
end;
...
var
  fontstr:string;
begin
  fontstr:=String(aFont.Name)+'|'+IntToStr(aFont.Size)+'|'+ColorToHex(FromMetaColor(aFont.Color));//bds2010
...
var
  ...
  begin
    ...
    begin
      Font.Name :=String(aFont.Name);//bds2010
  ...
  begin
    ...
    if (aObj.TextFlags and MetaTextFlagAutoSize)=MetaTextFlagAutoSize then
      ...
      case aObj.Alignment of
        MetaTextAlignmentCenter: Write(fWorkingFileHandle,'<div style="position:absolute;'+
                                                          'left:'+IntToStr((aObj.BoundsRect.Right+aObj.BoundsRect.Left-TextWidth(String(aObj.DisplayText),aObj.Font)) div 2)+'px;'+
                                                          'top:'+IntToStr(aObj.Origin.Y-fPrintCut.Y)+'px;'+
                                                          ClipStr(FromMetaRect(aObj.BoundsRect),cliprct)+'">');
        MetaTextAlignmentRight : Write(fWorkingFileHandle,'<div style="position:absolute;'+
                                                          'left:'+IntToStr(aObj.BoundsRect.Right-TextWidth(String(aObj.DisplayText),aObj.Font))+'px;'+
                                                          'top:'+IntToStr(aObj.Origin.Y-fPrintCut.Y)+'px;'+
                                                          ClipStr(FromMetaRect(aObj.BoundsRect),cliprct)+'">');
    ...
    // o texto
    Write(fWorkingFileHandle,EncodeISO(String(aObj.DisplayText)));//bds2010
    // retorna fonte
    if (aObj.Font.Style and MetaFontStyleStrikeOut)=MetaFontStyleStrikeOut then
      Write(fWorkingFileHandle,'</strike>');
    if (aObj.Font.Style and MetaFontStyleUnderline)=MetaFontStyleUnderline then
...
begin
  ...
  WriteLn(aFile,'<meta name="GENERATOR" content="'+CS_ProductTitleStr+' '+IntToStr(CommercialVersion)+'.'+IntToStr(ReleaseVersion)+'">');
  if Pages.Title<>'' then
...
begin
  ...
  try
    ...
    graphcrc:=CRC16(AnsiString(graphdat.DataString));//dbs2010
  finally
    graphdat.Free;
  end;
  i:=0;
