...
uses
  SysUtils, Classes, Math, Windows, Types, Graphics, Forms;
...
function  NewComponentName(aComponent:TComponent):AnsiString;
...
function  Token(const aTokenList:AnsiString; aIndex:integer; aTokenSeparator:AnsiChar='|'):AnsiString;overload;
function  Token(const aTokenList:String; aIndex:integer; aTokenSeparator:AnsiChar='|'):String;overload;
...
function  AddFileFilter(const aFilter, aDescription, aExt:String):String;

{@func GetFileFilterExt - Devolve a extensão padrão para arquivos correspondentes ao filtro aFilter. :/}
function  GetFileFilterExt(const aFilter:AnsiString; aIndex:integer):String;
...
{@func TextBounds - Calcula as dimensões do texto aText de acordo com a fonte aFont e opcionalmente rotacionado em
 ...
 @links PointsRect. :/}
function  TextBounds(const aText:AnsiString; aFont:TFont; aAngle:double):TPoint;
...
procedure CRC16Add(var Result:word; Data:AnsiChar);
function  CRC16(const Data; DataLen:integer):word; overload;
function  CRC16(const Str:AnsiString):word; overload;
...
begin
  ...
  try
    ...
    while i<l do
    begin
      stream.WriteString(String(AnsiChar(HexToByte(aHex[i]+aHex[i+1])))); //bds2010
      inc(i,2);
    end;
...
begin
  ...
  try
    ...
    // procura referência para a classe
    graphclassname:=AnsiString(UpperCase(String(graphclassname)));//bds2010
...
begin
  ...
  Result:=AnsiString(p+IntToStr(m+1));//bds2010
end;
...
function Token(const aTokenList:AnsiString; aIndex:integer; aTokenSeparator:AnsiChar='|'):AnsiString;
var
  i,m,count:integer;
begin
  Result:='';
...
function Token(const aTokenList:String; aIndex:integer; aTokenSeparator:AnsiChar='|'):String;
begin
   Result := String(Token(AnsiString(aTokenList), aIndex, aTokenSeparator));
end;
...
begin
  ...
  while i<=aIndex do
  begin
    m:=String(Token(aFilter,i,'|'));//bds2010
    p:=Pos('(',m);
    if p>0 then
      delete(m,1,p);
    p:=Pos(')',m);
...
begin
  ...
  try
    ...
    Result.x:=b.Canvas.TextWidth(String(aText));//bds2010
    Result.y:=b.Canvas.TextHeight(String(aText));//bds2010
    if aAngle<>0 then
    begin
      p[0]:=Point(0,0);
...
function CRC16(const Str:AnsiString):word;
var
  len:integer;
begin
  len:=Length(Str);
...
const
  MaxBuffer=16384;
var
  Buffer:packed array[0..MaxBuffer-1] of AnsiChar;
