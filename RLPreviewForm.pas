...
uses
  SysUtils, Math, Contnrs, Classes, Windows, Controls, Buttons, ExtCtrls,
  Forms, Dialogs, StdCtrls, Graphics, Types, RLConsts, RLMetaFile, RLPreview,
  RLFilters, RLUtils, RLPrintDialog, RLSaveDialog, RLPrinters, RLTypes,
  RLFindDialog;
...
type
  ...
  TRLPreviewForm = class(TForm)
    ...
    procedure SpeedButtonPriorMouseDown(Sender: TObject; Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
    procedure SpeedButtonPriorMouseUp(Sender: TObject; Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
    ...
    //Celso criei os dois métodos abaixo pq quando o foco estava no edit da página o Scroll com botão do meio do mouse não funcionava
  ...
  protected
    ...
    function DoMouseWheelDown(Shift: TShiftState; MousePos: TPoint): Boolean; override; //By Weber - Produsys Sistemas e Internet
    function DoMouseWheelUp(Shift: TShiftState; MousePos: TPoint): Boolean; override; //By Weber - Produsys Sistemas e Internet
...
begin
  with TOpenDialog.Create(nil) do
    try
      DefaultExt :=String(FormatFileExt(RLFILEEXT));//bds2010
      Filter     :=String(AddFileFilter('',CS_ProductTitleStr,RLFILEEXT));//bds2010
      FilterIndex:=1;
...
begin
  ...
  begin
    ...
    SpeedButtonFind := TSpeedButton.Create(Self);
    with SpeedButtonFind do
    begin
      Name := 'SpeedButtonFind';
      Parent := PanelTools;
      Left := 689;
      Top := 1;
      Width := 50;
    ...
    begin
      ...
      begin
        ...
        OnMouseWheelUp   := EditMouseWheelUp; //Celso
        OnMouseWheelDown := EditMouseWheelDown; //Celso
      end;
    end;
...
procedure TRLPreviewForm.EditMouseWheelDown(Sender: TObject; Shift: TShiftState; MousePos: TPoint; var Handled: Boolean);
begin
   Preview.ScrollDown;
end;

procedure TRLPreviewForm.EditMouseWheelUp(Sender: TObject; Shift: TShiftState; MousePos: TPoint; var Handled: Boolean);
begin
   Preview.ScrollUp;
end;
...
procedure TRLPreviewForm.SpeedButtonFindClick(Sender: TObject);
begin
  ShowFindDialog;
end;
...
procedure TRLPreviewForm.FormKeyPress(Sender: TObject; var Key: Char);
begin
  if (Key = '-') then // -10
  begin
    SpeedButtonZoomDownClick(Sender);
    Key := #0;
  end
  else if (Key = '+') or (Key = '=') then  // +10
  begin
    SpeedButtonZoomUpClick(Sender);
    Key := #0;
  end;
end;
...
function TRLPreviewForm.DoMouseWheelDown(Shift: TShiftState; MousePos: TPoint): Boolean;
begin
  Result := True;
  if (ssCtrl in Shift) then
    SpeedButtonZoomDownClick(nil)
  else
    Preview.ScrollDown;
end;

function TRLPreviewForm.DoMouseWheelUp(Shift: TShiftState; MousePos: TPoint): Boolean;
begin
  Result := True;
  if (ssCtrl in Shift) then
    SpeedButtonZoomUpClick(nil)
  else
    Preview.ScrollUp;
end;
